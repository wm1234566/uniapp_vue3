 ...
 * @LastEditTime: 2022-10-14 13:02:18
...
-->
<template>
  <page-meta root-font-size="52px" :style="{ display: 'block !important', overflowX: 'hidden' }">
    <view class="articles">
      <view class="detail-container">
        <videoItem :VideoItem="VideoItem" :isClick="false"></videoItem>

        <view class="comment-box">
          <view class="comment-limt-container">
            <view class="comment-title">精简评论</view>

            <view v-for="(item, index) in commentData" :key="item.info.avatar" class="comment-item-container">
              <view class="avatar-box">
                <image class="avatar" :src="item.info.avatar" mode="scaleToFill" />
              </view>
              <view class="info-box">
          ...
          <!-- 展示更多 -->
          <uni-load-more :color="color" :status="status" v-if="isShow" :content-text="contentText"
            @clickLoadMore="getList" />
        </view>

        <view class="operate-container">
          <view class="comment-box" @click="popup">
            <mySearch ref="mysearch" :Icon="inputIcon1" Showtext="评论一句，前排打call..."></mySearch>
          </view>
          <view class="options-box">
            <view class="praise-box">
              <image v-if="!articleData?.isPraise" class="img" src="@/static/images/un-praise.png" mode="scaleToFill" />

              <image v-else class="img" src="@/static/images/praise.png" mode="scaleToFill" />
          ...
          <veiw class="options-box">
            <view class="praise-box">
              <image v-if="!articleData?.isCollect" class="img" src="@/static/images/un-collect.png"
                mode="scaleToFill" />

              <image v-else class="img" src="@/static/images/collect.png" mode="scaleToFill" />
      ...
      <uni-popup ref="popup" type="bottom" background-color="#fff">
        <view class="comment-container">
          <textarea class="textarea" placeholder-style="color:#808080" placeholder="说点什么..." confirm-type="send" />
...
<script>
import { getArticleComment, userFollow } from "@/api/article.js";
...
export default {
  ...
  mounted() {
    console.log("Vuexsdfds", this.VideoItem);
  },
  components: {
    mySearch,
